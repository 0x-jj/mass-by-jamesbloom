...
contract ScriptyHTML is ScriptyCore, IScriptyHTML {
   ...
   */
  function getHTML(HTMLRequest memory htmlRequest) public view returns (bytes memory) {
    // calculate buffer size for requests
    uint256 requestBufferSize;
    unchecked {
      if (htmlRequest.headTags.length > 0) {
        requestBufferSize = _enrichHTMLTags(htmlRequest.headTags, false);
      }

      if (htmlRequest.bodyTags.length > 0) {
        requestBufferSize += _enrichHTMLTags(htmlRequest.bodyTags, false);
      }
    }

    bytes memory htmlFile = DynamicBuffer.allocate(_getHTMLBufferSize(requestBufferSize));
   ...
   */
  function _getHTMLBufferSize(uint256 requestBufferSize) private pure returns (uint256 size) {
    unchecked {
   ...
   */
  function getEncodedHTML(HTMLRequest memory htmlRequest) public view returns (bytes memory) {
    unchecked {
   ...
   */
  function getHTMLString(HTMLRequest memory htmlRequest) public view returns (string memory) {
   ...
   */
  function getEncodedHTMLString(HTMLRequest memory htmlRequest) public view returns (string memory) {
