...
contract FileStore is IFileStore {
   ...
   */
  function getPointer(string memory filename) public view returns (address pointer) {
   ...
   */
  function _fileFromContents(string memory contents) internal returns (File memory file) {
   ...
   */
  function _fileFromChunks(string[] memory chunks) internal returns (File memory file) {
    uint256 size = 0;
    ...
    for (uint256 i = 0; i < chunks.length; ++i) {
      ...
      slices[i].end = uint32(SSTORE2.DATA_OFFSET + bytes(chunks[i]).length);
   ...
   */
  function _fileFromPointers(address[] memory pointers) internal view returns (File memory file) {
    uint256 size = 0;
   ...
   */
  function _fileFromSlices(BytecodeSlice[] memory slices) internal view returns (File memory file) {
    uint256 size = 0;
    for (uint256 i = 0; i < slices.length; ++i) {
      if (slices[i].end - slices[i].start <= 0) {
        revert SliceEmpty(slices[i].pointer, slices[i].start, slices[i].end);
      }
      uint32 codeSize = uint32(slices[i].pointer.code.length);
      if (slices[i].end > codeSize) {
        revert SliceOutOfBounds(slices[i].pointer, codeSize, slices[i].start, slices[i].end);
      }
      size += slices[i].end - slices[i].start;
    }
    return File({size: size, slices: slices});
   ...
   */
  function readFile(string memory filename) public view returns (string memory contents) {
